...
TODO: *Check which (if either) of these is correct - then edit all places where this is used again later in lesson*
```
...
> ## Challenge: Bad names for objects 
...
TODO: *Touch on data types? Determine about of detail to include for types of data in R at this stage*
TODO: *Explain how to add comments that are not run?* 
...
TODO: *I think delete bellow error message - should appear automatically from above code chunk?* 
```
...
You pass **arguments** to a function by placing values in between the parenthesis. Notice in our example, our command contained `file = "SoftwareCarpentry/SoftwareCarpentry09_05/myData/gapminder_data.csv"` inside the `read_csv()` function parenthasis. Here `file` is the argument name, and `"SoftwareCarpentry/SoftwareCarpentry09_05/myData/gapminder_data.csv"` is the value we assigned to that argument. 

TODO: *skip this and talk about it later?* 
When you need to pass in multiple arguments to a function you will separate those values with commas. Often we can skip the argument name, if the argument values are passed in a certain order. But that's just a quick summary -- if that all sounds confusing, we will see examples of those later.

TODO: *keep as exercise or just regular text?* 
> ## Exercise: Do all functions need arguments? 
> Let's test some other functions. What happens when we run the following lines of code? 
> 
>```{r Functions}
>  Sys.Date()
>  getwd()
...
>> ## Solution 
>>
>> The first line of code should output the current date, and the second line of code gives your working directory. 
>>
> {: .solution}
{: .challenge}


Each function has a help page that documents what arguments the function expects and what value it will return. You can bring up the help page a few different ways. If you have typed the function name in the **Editor** windows, you can put your cursor on the function name and press <kbd>F1</kbd> to open help page in the **Help** viewer in the lower right corner of RStudio. You can also type `?` followed by the function name in the console. For example, try running `?read_csv`. A help page should pop up with information about what the function is used for and how to use it, as well as useful examples of the function in action.
...
TODO: *Keep or cut?*
> ## What are **aesthetics**? 
> The elements of a plot have a bunch of properties like an x and y position, a size, a color, etc.These properties are called **aesthetics**. When creating a data visualization, we must map a variable in our dataset to an aesthetic in our plot. For example, in a scatterplot of US universities, we may map the `student_population` variable of the university to the size aesthetic of our points, so that universities with a large population have larger points. 
>
{: .callout}

To create our plot, we need to map variables from our `gapminder_1997` object to ggplot aesthetics using the `aes()` funtion. Since we have already told `ggplot` that we are using the data in the `gapminder_1997` object, we can access the columns of `gapminder_1997` using the object's column names. (Remember, R is case-senstitive, so we must be careful to match the comlumn names exactly!) 
...
TODO: *Do they know about doing math with vectors/objects yet?*
...
TODO: *show how the function is inside the other function?*. 
...
TODO: *cut this if lesson is too long?* 
## Facets
...
saving plot and adding things to it 
myplot <- ggplot(gapminder_data, aes(x = lifeExp, y = gdpPercap)) + geom_point()
myplot + theme_bw()
