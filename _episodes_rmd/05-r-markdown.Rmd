...
We can also make a table directly from our data. To do so we will use the `kable()` function. This function allows for customization of the output table. To see all the options you can look at the help `?kable` or search online.

```{r,comment=NA,echo=F,collapse=T}
cat('```{r data_summary, include=FALSE}\n
  summary_table <- gapminder_1997 %>% \n
    group_by(continent) %>% \n
    summarise(nCountries = length(country),\n
      minGDP = round(min(gdpPercap), digits = 0),\n
      maxGDP = round(max(gdpPercap), digits = 0))\n
