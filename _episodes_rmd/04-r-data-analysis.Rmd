...
<-- To move to bonus content -->

> ## Bonus Exercise: Using helper functions with `select()`
>
...
> ## Bonus Exercise: Sort data with `arrange()`
>
...
<-- Move creation of final gapminder_data_2007 as exercise here --> 


## Reviewing Git and GitHub
Great! Now that we have our gapminder data prepared, let's use what we learned about git and GitHub in the previous lesson to add, commit, and push our changes.  
...
```{r RecodeJoinExercise2, include=FALSE}
gapminder_data <- read_csv("data/gapminder_data.csv") %>%
  filter(year == 2007 & continent == "Americas") %>%
  select(-year, -continent) %>%
  mutate(country = recode(country, "Puerto Rico" = "United States")) %>%
