...
We name this new column so we can use in a future argument. So the `averageLifeExp=` part tells `summarize()` to use "averageLifeExp" as the name of the new column.
...
```
There are often multiple ways to clean data. Another way to deal with the warning message is given in the bonus content, if you're interested in checking it out. 
...
Now we're ready to join our CO2 emissions data to the `gapminder_data_2007` dataframe we made earlier. 
...
The dplyr package has a number of tools for joining data frames together depending on what we want to do with the rows of the data of countries that are not represented in both data frames. Let's look at some cartoon examples and then come back to our own data. Here we'll talk about `inner_join()` and `anti_join()`, but other options are discussed in the bonus content. 
...
## Second method of changing the column names of the CO2 dataframe 
Here we  read in the table, get the warning and then fix the column names using the rename function.
...
## Finding the year 2005 

Here we show how we decided to use data from 2005 in the CO2 dataset. 

One useful tool is the `count()` function, which will tell us how many times a value is repeated in a column of a data frame. Let's use this function on the year column to see which years we have data for and to tell us whether we have a good number of countries represented in that year.
